import warp as wp
from msk_warp import SwingTwistLimit
from msk_warp.utils.converted_objects import SwingTwistLimitData
from msk_warp.utils.xml_helper import extract_msk_only_objects, extract_float_from_element, extract_string_from_element, \
    extract_vec3_from_element


def convert_swing_twist_limits(model_path: str) -> list[SwingTwistLimitData]:
    """ Extracts swing twist limit data from the MSKOnlySet in the OpenSim model XML. """
    swing_twist_data = []
    msk_only_objects = extract_msk_only_objects(model_path)
    if msk_only_objects is None:
        return swing_twist_data

    for obj in msk_only_objects:
