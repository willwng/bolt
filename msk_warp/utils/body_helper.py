...
def _shift_inertia_from_com_to_body(mass: float, mass_center: np.ndarray, inertia_in_com: np.ndarray) -> np.ndarray:
    ...
    mxx = mp[0] * to_body_center[0]
    myy = mp[1] * to_body_center[1]
    mzz = mp[2] * to_body_center[2]
    nmx = -mp[0]
    nmy = -mp[1]
    point_mass = np.array([[myy + mzz, nmx * to_body_center[1], nmx * to_body_center[2]],
                           [nmx * to_body_center[1], mxx + mzz, nmy * to_body_center[2]],
                           [nmx * to_body_center[2], nmy * to_body_center[2], mxx + myy]])
...
def convert_body(body: OSimType.Body) -> BodyData:
    ...
    unit_inertia_in_B_warp = wp.mat33(
