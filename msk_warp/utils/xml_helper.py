...
def extract_msk_only_objects(model_path: str):
    # OpenSim doesn't support some features, so we put them in a custom set called MSKOnlySet
    tree = ET.parse(model_path)
    root = tree.getroot()
    model = root.find("Model")
    extra_set = model.find("MSKOnlySet")
