...
def evaluate_polynomial(
    ...
    # TODO: don't hard code this for 6 dofs
    coeff_idx = int(0)
        ...
        for in2 in range(max_in2 + 1):
            ...
            for in3 in range(max_in3 + 1):
                ...
                for in4 in range(max_in4 + 1):
                    max_in5 = (order - in1 - in2 - in3 - in4) if n_dof >= 5 else 0
                    for in5 in range(max_in5 + 1):
                        max_in6 = (order - in1 - in2 - in3 - in4 - in5) if n_dof >= 6 else 0
                        for in6 in range(max_in6 + 1):
                            c = coefficients[start_idx + coeff_idx]
                            t1 = q_pows[0, in1]
                            t2 = q_pows[1, in2]
                            t3 = q_pows[2, in3]
                            t4 = q_pows[3, in4]
                            ...
                            # Function eval
                            term_all = t1 * t2 * t3 * t4 * t5 * t6
                            length += c * term_all

                            # Partial Derivatives (dL/dq)
                            if in1 > 0:
                                df_dq[0] += c * float(in1) * q_pows[0, in1 - 1] * t2 * t3 * t4 * t5 * t6
                            if in2 > 0:
                                df_dq[1] += c * t1 * float(in2) * q_pows[1, in2 - 1] * t3 * t4 * t5 * t6
                            if in3 > 0:
                                df_dq[2] += c * t1 * t2 * float(in3) * q_pows[2, in3 - 1] * t4 * t5 * t6
                            if in4 > 0:
                                df_dq[3] += c * t1 * t2 * t3 * float(in4) * q_pows[3, in4 - 1] * t5 * t6
                            if in5 > 0:
                                df_dq[4] += c * t1 * t2 * t3 * t4 * float(in5) * q_pows[4, in5 - 1] * t6
                            if in6 > 0:
                                df_dq[5] += c * t1 * t2 * t3 * t4 * t5 * float(in6) * q_pows[5, in6 - 1]
