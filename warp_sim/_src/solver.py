...
def _eval_init(
    ...
    for efcid in range(nefc_clip):
        if type_in[efcid] == types.ConstraintType.CONTACT_ELLIPTIC:
            ...
            lo += _eval_elliptic(impratio_invsqrt, friction, efc_quad0,
                                 efc_quad1, efc_quad2, alpha)
        else:
...
def _eval(
    ...
    for efcid in range(nefc_clip):
        if type_in[efcid] == types.ConstraintType.CONTACT_ELLIPTIC:
            ...
            lo += _eval_elliptic(impratio_invsqrt, friction, efc_quad0,
                                 efc_quad1, efc_quad2, lo_alpha)
            hi += _eval_elliptic(impratio_invsqrt, friction, efc_quad0,
                                 efc_quad1, efc_quad2, hi_alpha)
            mid += _eval_elliptic(impratio_invsqrt, friction, efc_quad0,
                                  efc_quad1, efc_quad2, mid_alpha)
        else:
            Jaref = Jaref_in[efcid]
            jv = jv_in[efcid]
            quad = quad_in[efcid]
...
@wp.func
def _log_scale(min_value: float, max_value: float, num_values: int,
               i: int) -> float:
    step = (wp.log(max_value) - wp.log(min_value)) / wp.max(1.0, float(
        num_values - 1))
...
def linesearch_parallel_fused(
    ...
    alpha = _log_scale(opt_ls_parallel_min_step, 1.0, opt_ls_iterations,
                       alphaid)
    ...
    for efcid in range(min(njmax_in, nefc_in[worldid])):
        ...
        # limit and contact
            ...
            worldid, efcid] == types.ConstraintType.CONTACT_ELLIPTIC:
            ...
            friction = contact_friction_in[conid]
            mu = friction[0] / wp.sqrt(
                opt_impratio[worldid % opt_impratio.shape[0]])
...
def linesearch_parallel_best_alpha(
    ...
    efc_alpha_out[worldid] = _log_scale(opt_ls_parallel_min_step, 1.0,
                                        opt_ls_iterations, bestid)
...
def _linesearch_parallel(m: types.Model, d: types.Data,
    ...
    wp.launch(
        ...
        dim=(d.nworld),
        inputs=[m.opt.ls_iterations, m.opt.ls_parallel_min_step, d.efc.done,
                cost],
        outputs=[d.efc.alpha],
    )
    ...
    def kernel(
        ...
        if wp.static(dofs_per_thread >= nv):
            for i in range(wp.static(min(dofs_per_thread, nv))):
                jv_out += efc_J_in[worldid, efcid, i] * efc_search_in[
                    worldid, i]
            ...
            for i in range(wp.static(dofs_per_thread)):
                ...
                if ii < nv:
                    jv_out += efc_J_in[worldid, efcid, ii] * efc_search_in[
                        worldid, ii]
...
def linesearch_prepare_quad(
    ...
    quad = wp.vec3(0.5 * Jaref * Jaref * efc_D, jv * Jaref * efc_D,
                   0.5 * jv * jv * efc_D)
...
def linesearch_jaref(
    ...
    efc_Jaref_out[worldid, efcid] += efc_alpha_in[worldid] * efc_jv_in[
        worldid, efcid]
...
def _linesearch(m: types.Model, d: types.Data, cost: wp.array2d(dtype=float)):
    ...
    wp.launch(
        ...
        dim=(d.nworld),
        inputs=[m.nv, d.qfrc_smooth, d.efc.Ma, d.efc.search, d.efc.gauss,
                d.efc.mv, d.efc.done],
...
def update_constraint_efc(
    ...
    if efc_type_in[worldid, efcid] != types.ConstraintType.CONTACT_ELLIPTIC:
        ...
        # middle zone
            ...
            dm = math.safe_div(efc_D_in[worldid, efcid0],
                               mu * mu * (1.0 + mu * mu))
            nmt = N - mu * T
            ...
            if efcid == efcid0:
                ...
                efc_force_out[worldid, efcid] = -math.safe_div(force,
                                                               T) * ufrictionj
...
def update_constraint_gauss_cost(nv: int, dofs_per_thread: int):
    ...
    def kernel(
        ...
        if wp.static(dofs_per_thread >= nv):
            for i in range(wp.static(min(dofs_per_thread, nv))):
                gauss_cost += (efc_Ma_in[worldid, i] - qfrc_smooth_in[
                    worldid, i]) * (qacc_in[worldid, i] - qacc_smooth_in[
                    worldid, i])
            ...
            for i in range(wp.static(dofs_per_thread)):
                ii = dofstart * wp.static(dofs_per_thread) + i
                if ii < nv:
                    gauss_cost += (efc_Ma_in[worldid, ii] - qfrc_smooth_in[
                        worldid, ii]) * (
                                          qacc_in[worldid, ii] - qacc_smooth_in[
                                      worldid, ii]
                                  )
...
def update_gradient_grad(
    ...
    grad = efc_Ma_in[worldid, dofid] - qfrc_smooth_in[worldid, dofid] - \
           qfrc_constraint_in[worldid, dofid]
...
def update_gradient_JTDAJ_sparse_tiled(tile_size: int, njmax: int):
    ...
    def kernel(
        ...
        for k in range(0, njmax, TILE_SIZE):
            ...
            J_ki = wp.tile_load(efc_J_in[worldid], shape=(TILE_SIZE, TILE_SIZE),
                                offset=(k, offset_i), bounds_check=False)

            if offset_i != offset_j:
                J_kj = wp.tile_load(efc_J_in[worldid],
                                    shape=(TILE_SIZE, TILE_SIZE),
                                    offset=(k, offset_j), bounds_check=False)
            else:
                wp.tile_assign(J_kj, J_ki, (0, 0))

            D_k = wp.tile_load(efc_D_in[worldid], shape=TILE_SIZE, offset=k,
                               bounds_check=False)
            state = wp.tile_load(efc_state_in[worldid], shape=TILE_SIZE,
                                 offset=k, bounds_check=False)

            D_k = wp.tile_map(state_check, D_k, state)

            # force unused elements to be zero
            tid_tile = wp.tile_arange(TILE_SIZE, dtype=int)
            threshold_tile = wp.tile_ones(shape=TILE_SIZE, dtype=int) * (
                        nefc - k)

            active_tile = wp.tile_map(active_check, tid_tile, threshold_tile)
            D_k = wp.tile_map(wp.mul, active_tile, D_k)

            J_ki = wp.tile_map(wp.mul, wp.tile_transpose(J_ki),
                               wp.tile_broadcast(D_k,
                                                 shape=(TILE_SIZE, TILE_SIZE)))
...
def update_gradient_JTDAJ_dense_tiled(nv: int, tile_size: int, njmax: int):
    ...
    def kernel(
        ...
        sum_val = wp.tile_load(qM_in[worldid], shape=(nv, nv),
                               bounds_check=False)
        ...
        for k in range(0, njmax, TILE_SIZE_K):
            ...
            # everything always hits the fast path. The padding takes care of any
            #  potential OOB accesses.
            J_ki = wp.tile_load(efc_J_in[worldid], shape=(TILE_SIZE_K, nv),
                                offset=(k, 0), bounds_check=False)
            J_kj = J_ki

            # state check
            D_k = wp.tile_load(efc_D_in[worldid], shape=TILE_SIZE_K, offset=k,
                               bounds_check=False)
            state = wp.tile_load(efc_state_in[worldid], shape=TILE_SIZE_K,
                                 offset=k, bounds_check=False)
            ...
            J_ki = wp.tile_map(wp.mul, wp.tile_transpose(J_ki),
                               wp.tile_broadcast(D_k, shape=(nv, TILE_SIZE_K)))
...
def update_gradient_JTCJ(
    ...
    for i in range(nblocks_perblock):
        ...
        mu = math.safe_div(fri[0], wp.sqrt(opt_impratio))
...
def update_gradient_cholesky_blocked(tile_size: int):
    ...
    def kernel(
        ...
        wp.static(create_blocked_cholesky_func(TILE_SIZE))(tid_block,
                                                           efc_h_in[worldid],
                                                           matrix_size,
                                                           cholesky_L_tmp[
                                                               worldid])
        wp.static(create_blocked_cholesky_solve_func(TILE_SIZE))(
            tid_block, cholesky_L_tmp[worldid], efc_grad_in[worldid],
            cholesky_y_tmp[worldid], matrix_size, efc_Mgrad_out[worldid]
        )
...
def _update_gradient(m: types.Model, d: types.Data):
    ...
    wp.launch(update_gradient_zero_grad_dot, dim=(d.nworld),
              inputs=[d.efc.done], outputs=[d.efc.grad_dot])
    ...
    if m.opt.solver == types.SolverType.CG:
        ...
        wp.launch_tiled(
            update_gradient_JTDAJ_dense_tiled(m.nv,
...
def solve_done(
    ...
    improvement = _rescale(nv, stat_meaninertia,
                           efc_prev_cost_in[worldid] - efc_cost_in[worldid])
    gradient = _rescale(nv, stat_meaninertia,
                        wp.math.sqrt(efc_grad_dot_in[worldid]))
...
def _solver_iteration(
    ...
    if m.opt.solver == types.SolverType.CG:
        wp.launch(
            solve_beta,
            dim=d.nworld,
            inputs=[m.nv, d.efc.grad, d.efc.Mgrad, d.efc.prev_grad,
                    d.efc.prev_Mgrad, d.efc.done],
            outputs=[d.efc.beta],
        )

    wp.launch(solve_zero_search_dot, dim=(d.nworld), inputs=[d.efc.done],
              outputs=[d.efc.search_dot])

    wp.launch(
        solve_search_update,
        dim=(d.nworld, m.nv),
        inputs=[m.opt.solver, d.efc.Mgrad, d.efc.search, d.efc.beta,
                d.efc.done],
...
def _solve(m: types.Model, d: types.Data):
    ...
    step_size_cost = wp.empty(
        (d.nworld, m.opt.ls_iterations if m.opt.ls_parallel else 0),
        dtype=float)
