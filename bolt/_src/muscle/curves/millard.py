""" Millard force curves """
import warp as wp

from bolt import consts
from bolt.types import vec6
...
def calc_quintic_bezier_corner_control_points(
    ...
    root_eps = wp.sqrt(consts.BOLT_MINVAL)
...
def evaluate_quintic_bezier_der_x(u: float, x_pts: vec6, y_pts: vec6) -> float:
    ...
    dy_dx = 0.0
    if wp.abs(dx_du) > consts.BOLT_SIG_REAL:
        dy_dx = dy_du / dx_du
    return dy_dx
...
def compute_u(x: float, x_pts: vec6) -> float:
    ...
    for _ in range(max_iter):
        ...
        error = current_x - x
        if wp.abs(error) < consts.BOLT_SIG_REAL:
            break
        dxdu = evaluate_quintic_bezier_der_u(u, x_pts)
        if wp.abs(dxdu) < consts.BOLT_SIG_REAL:
            break
        u -= error / dxdu
