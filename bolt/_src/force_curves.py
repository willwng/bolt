...
def calc_damped_norm_fiber_velocity(
    ...
    tol = wp.max(1e-10 * f_iso, consts.BOLT_SIG_REAL * 100.0)
    err = float(1e10)
    i = int(0)
    ...
    while wp.abs(err) > tol and i < max_iter:
        ...
        if wp.abs(err) > tol and wp.abs(derr_d_dlceNdt) > consts.BOLT_SIG_REAL:
            delta = -err / derr_d_dlceNdt
            dlceN_dt = dlceN_dt + delta
        elif wp.abs(derr_d_dlceNdt) < consts.BOLT_SIG_REAL:
