...
def attempt_adaptive_step(m: Model, d: Data):
    ...
    adaptive_common.save_state_idx(m, d, 1)

    # Restore y_0 (note that y_0' is unmodified after restore). Take two half steps
    adaptive_common.restore_state_idx(m, d, 0, only_on_reject=False)
    ...
    adaptive_common.compute_error(m, d, d.integrator_scratch[1], scale=1.0)
    ...
    adaptive_common.restore_state_idx(m, d, 0, only_on_reject=True)
